...
namespace PressurizedPipes.HarmonyPatches
{
    public static class CustomPatcher
    {
        ...
        public static void PostPatch(HarmonyInstance instance)
        {
            ...
            TryBiggerCapacityIntegrationPatch(instance);
        }
        ...
        //Compatibility Patch for: Bigger Capacity by newman55
        private static void TryBiggerCapacityIntegrationPatch(HarmonyInstance instance)
        {
            HarmonyInstance customInstance = HarmonyInstance.Create("pressurized_pipes.super_corgi");
            Type patchType = AccessTools.TypeByName("BiggerCapacity.BiggerCapacity+ValveBase_OnSpawn_Patch");
            if (patchType == null)
            {
                Debug.Log($"[Pressurized] Could not find Bigger Capacity mod");
                return;
            }
            Debug.Log("[Pressurized] Bigger Capacity mod detected. PressurizedPipes will attempt to integrate compatibility.");
            MethodInfo biggerCapacityTarget = AccessTools.Method(typeof(ValveBase), "OnSpawn");
